# Trains a gcncheb network in batches (use: python3 thisFileName datasetName) ex. dataset medium9_2
...
# Net class
from Networks.GCN.chebnetbatch import GCN
# Pytorch imports
import torch
...
try:
    ...
    def decay(lr, d, epoch):
        return learning_rate*0.25**(np.floor(epoch/2))
    
    lambdadecay = lambda epoch : 0.5**(np.floor(epoch/2))
    ...
    def batchActions():
        ...
        ptb_input = Batch().from_data_list(batch_input)
        ptb_input = ptb_input.to(device)
    ...
    def testAction():
        ...
        # Batch to tensor
        tb_input = Batch().from_data_list(testBatch_input)
        tb_input = tb_input.to(device)
    ...
    for epoch in epochs:
        ...
        for index,i in enumerate(train_index):
            ...
            batch_input.append(Data(x=getFeatures(init_img_state,goal_img_state),edge_index=g))
            ...
            for t in range(T):
                ...
                batch_input.append(Data(x=getFeatures(step_img_state,goal_img_state),edge_index=g))
            ...
            for k in indices_bootstrap:
                ...
                #goal_step[:,:,3] *= False
                batch_input.append(Data(x=getFeatures(bootstrap[k[0]],goal_step),edge_index=g))
    ...
    for index,i in enumerate(test_index):
        ...
        testBatch_input.append(Data(x=getFeatures(init_img_state,goal_img_state),edge_index=g))
        ...
        for t in range(T):
            ...
            step_img_state = env.get_observation()
            # bootstrap.append(step_img_state)
            testBatch_input.append(Data(x=getFeatures(step_img_state,goal_img_state),edge_index=g))
        ...
        #     if mb_idx == minibatch:
        #         batchActions()
    torch.save(net.state_dict(), "batchecheb-pred.pth")
    print(sum(p.numel() for p in net.parameters() if p.requires_grad))
    print("learning rate {}".format(learning_rate))
    print("4 as cnnrelu-skip layers")
